...
export function createUnderlinedDropdown<T>({ render, format }: UnderlinedDropdownConfig<T>) {
  return class UnderlinedDropdown extends React.Component<UnderlinedDropdownProps<T>, { visible: boolean }> {
    state = { visible: false }
    ...
    render() {
      return (
        <>
          <a href="#" onClick={this.handleClick}>{format(this.props.value)}</a>
          <Popover
