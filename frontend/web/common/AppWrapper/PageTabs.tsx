...
export const PageTabs = (props: PageTabsProps) => (
  ...
  {
    ({ location, history }) => {
      const pages = React.Children.toArray(props.children) as Array<React.ReactElement<PageTabProps>>
      const value = pages.findIndex(page => page.props.path === location.pathname)

      return (
        <Tabs {...props} value={value}>
        {
          pages.map(page => React.cloneElement(page, {
