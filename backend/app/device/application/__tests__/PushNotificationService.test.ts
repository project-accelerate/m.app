...
import { somePushNotification, someFailedPushNotificationReceipt, someSuccessfulPushNotificationReceipt } from "../../test/expoPushClientTestUtils";
...
describe(PushNotificationService, () => {
  describe('sendNotifications', () => {
    ...
    it('should send notifications and save record of sending', async () => {
      ...
      fixture.pendingNotificationRepository
        .givenIdReturnedFromInsert(notificationId)
      ...
      fixture.verifyNotificationsSent(notification)
  ...
  describe('checkNotificationReceipts', () => {
    ...
    it('should unregister device and remove pending notification if notification failed due to device becoming inactive', async () => {
      ...
      fixture.pendingNotificationRepository
        .givenObjectsReturnedFromFindAll([pendingNotification])

      fixture.givenANotificationReceipt(pendingNotification.ticketId,
        someFailedPushNotificationReceipt({ details: { error: 'DeviceNotRegistered' } })
      )
    ...
    it('should remove pending notification but not unregister device and if notification failed for some other reason', async () => {
      ...
      fixture.givenANotificationReceipt(pendingNotification.ticketId,
        someFailedPushNotificationReceipt({ details: { error: 'InvalidCredentials' } })
      )

      await fixture.service.checkNotificationReceipts()

      fixture.deviceAdminService
...
class Fixture {
  ...
  pendingNotificationRepository = new MockCrudRepositoryFixture(PendingNotificationRepository)
  ...
  constructor() {
    when(this.expoClient.requestNotificationReceipts(anything()))
      .thenResolve({})
  }

  verifyNotificationsSent(message: ExpoPushMessage) {
    verify(this.expoClient.sendNotifications(deepEqual([message])))
      .called()
  }

  givenANotificationReceipt(ticketId: string, receipt: ExpoPushReceipt) {
    when(this.expoClient.requestNotificationReceipts(deepEqual([ticketId])))
      .thenResolve({
        [ticketId]: receipt
      })
  }
  ...
  givenTicketIdForSentNotifcation(id: string) {
    when(this.expoClient.sendNotifications(anything()))
      .thenResolve([
        {
          id
        }
