...
export class ExpoPushClient {
  ...
  async sendNotifications(notifications: ExpoPushMessage[]) {
    ...
    if (chunks.length > 1) {
      log.warn(
        `[Push] batch exceeds a single chunk (chunks: ${
          chunks.length
    ...
    for (let chunk of chunks) {
      try {
        log.debug('[Push] Sending notifications to devices', chunk)
  ...
  async requestNotificationReceipts(receiptIds: ExpoPushReceiptId[]) {
    ...
    if (chunks.length > 1) {
      log.warn(
        `[Push] Receipt batch exceeds a single chunk (chunks: ${
          chunks.length
        }, chunkSize: ${chunks[0].length})`,
      )
    }
    ...
    for (let chunk of chunks) {
      try {
        const chunkReceipts = await this.expo.getPushNotificationReceiptsAsync(
          chunk,
        )
